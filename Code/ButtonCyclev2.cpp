...
void colorWipe(uint32_t color, int wait) {
  for(int i=0; istrip.numPixels(); i++) {
...
void rainbow(int wait) {
  for(long firstPixelHue = 0; firstPixelHue  565536; firstPixelHue += 256) {
...
void handleButtonPress(int button) {
  ...
  if (newState == LOW && oldState == HIGH) {
    delay(50);  debounce
      if (button == BUTTON) {
        mode += 1;   Increase mode for button 1
        if (mode == 4){
          mode = 1;
        }
      }
...
if (current_mode != mode)
{
  switch (mode) {
        case 1
          ...
          break;
        case 2
          colorWipe(strip.Color(62, 0, 0), 50);
          break;
        case 3
          blueWaveSignal(50);
          colorWipe(strip.Color(00, 44, 0), 50);
          break;
         case 4
           blueWaveSignal(50);
           break;
