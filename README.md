...
<h1 id="how">How does it work?</h1>
    <p>The 17 LEDstrips are controlled by an ESP32-C3 microcontroller. That sends signals to the strips to recreate a handful of patterns. At the side of the box the visitors can push on 2 different buttons which put the LEDs in different modes.</p>

<H1>Hardware</H1>
    The "hardware" of the project is described in this chapter.
<h2 id="require">Hardware Requirements</h2>
    <ul>
        <li>+/- 10 meters of sk6812 rgwb LEDstrips</li>
...
<h2>Box</h2>
    To build the box, we used MBF plates. These are strong wooden plates that can easily be drilled into. The inside of the box has the dimension 70cm x 70cm x 70cm. This is the space where someone can stand in when they want to take a picture. On the inside, we used mirror foil to create the illusion of a bigger number of LEDs. The buttons are placed at the bottom side for easy access when taking a picture. 

<h2>LEDstrips</h2>
    In the current project, we use 17 LEDstrips which are each +/- 50 cm. This is a good length for the current height of the box (70 cm), as such we didn't need an extra 5m led strip just to use 20% of it. To know if using 17 led strips works, we first tested it by using 2 data pins to control all the LEDstrips:
...
<ul>
    <li>
        In this project, all LEDstrips hang on their own. This means that the side where LEDs are, are faced to the public or to the mirrors on the inside. They are never facing both. This is not a big problem but interferes with the illusion of the mirrors. This problem could be solved by putting 2 LEDstrips back to back, instead of hanging them alone.
    </li>
    <li>
        Each LEDstrip is connected directly to the ESP32. This means that there is maximum 3.3V on the data pin when sending a high signal. This can be the reason why the LEDs are not always working as intended. This can be solved by using a transistor or a MOSFET to amplify the signal. 
    </li>
</ul>
<h1>Software</h1>
    <h2>Adafruit_NeoPixel library</H2>
        ...
        Now we just need to import the Adafruit_NeoPixel.h library into our project (included inside the main.cpp file). You can simply do this by searching the Adafruit Neopixel library in the Libraries menu (at the left side in PIO Home). You should now be able to use the code on the ESP32 by building it and uploading it!
    <h2>main.cpp</h2>
        In the main program file for the project, there are 3 crucial parts:
        <ul>
            <li>
                <h3>Buttons</h3>
                The buttons are programmed to be triggered on the positive flank. This means that when the button is going from a low state to a high state, the program will interpret that as a button press. This means that nothing will happen when someone releases the button or holds it. To reduce the dander, there is a delay programmed to filter only one positive flank.
            </li>
            <li>
                <h3>Cycle</h3>
                The cycle is the part of the program that is responsible for changing the pattern. When a button is pressed, the cycle will change to the next pattern. This is done by using an array which includes all patterns where the index, which is used to filter the correct data, is increased or decreased. When the index is bigger than the array, the index will be set to 0. When the index is smaller than 0, the index will be set to the last index of the array.
            </li>
            <li>
                <h3>Sending data</h3>
                To send the data to the LEDstrips, the correct data has to be filtered. This is done by using a series of if-statements which checks the data in the array from above. when the correct data is found, the data will be sent to the ledstrips. This is done by using the Adafruit_NeoPixel library. This library has a function called 'show()' which will send the data to the LEDstrips. Since there are 4 data pins, the sent data looks like this:
    ...
    <ul>
    <li><b>Rain:</b></li> The rain effect shows rain falls from 4 LEDs in a blue color. Each datapin has a delay so the drops are falling irregular.
    <li><b>Wave fade:</b></li> The wave fade effect shows a blue wave in the LEDs.
    <li><b>Blinking star light:</b></li> This effect simulates starlight that go on and off in a white color.
    <li><b>Sunrise:</b></li> Sunset colors are slowly waving on the LEDstrips.
    <li><b>Ripple:</b></li> The LEDs are jumping up and down.
    ...
    There are 4 topic that can be sent to the MQTT broker:
    <ul>
    <li>
    ...
    <li><b>Effect</b></li> The effect topic shows the selected effect from 7 possible effects.
    </ul>
<h1>Project Expansion</h1>
    ...
    <h2>Ideas (more details below)</h2>
        <ul>
            ...
            <li>using real mirrors instead of foil</li>
            <li>bigger microcontroller</li>
            <li>PCB</li>
            <li>...</li>
        </ul>
    <h2>Our vision</h2>
    As you can see up here, there are a lot of things one can do to make this a incredible project expansion. Under here, we will explain the ideas we think are the most important to make this project a success. Here is our vision of what this project could become.
    <h3>Box</h3>
    <ul>
        <li>
            <h3>Dimensions</h3>
        ...
        <li>
            <h3>Mirrors</h3>
            To create the effect that we want, the top and sides of the inside of the box has to be reflective. The best thing to use for this would be -of course- mirrors. In the prototype, we did not use real mirrors because of the expensive price. Another reason we didn't choose real mirrors is the fact that the LEDstrips have to come through the top, this would be very hard to do. Drilling holes in a mirror is a difficult process and will result in a huge money loss if it goes wrong. So for the top part, we would stick to the mirror foil. For the side parts, real mirrors could be considered. While the mirror foil is a cheap and easy way to create the wanted effect, it is not as good as a real mirror. The mirror foil is not as reflective as a real mirror, so the effect is less spectacular.
        </li>
    </ul>
    <h3>LEDstrips</h3>
        When the box becomes bigger, it is normal that the number and length of the ledstrips will increase too. If we want to make a box as described above, a number of 25 ledstrips hanging from above would be perfect in our vision. Since we wanted to double the LEDstrips for the mirror effect (see chapter Hardware), this would come down to 50 ledstrips. When we make each of them 150 cm long, that would mean that there would be 75 meters of ledstrips. This would come down to 277 euro with the type of LEDs we are currently using.
        <img src="src/expantion_ledplacement.PNG" alt="expanded box">
        This would be a good placement of the LEDstrips. With the staggered placement, the mirrors will reflect as much of the light as possible resulting in a beautiful photo.
    <h3>Electronics</h3>
    <ul>
        <li>
            <h3>Microcontroller</h3>
                The best way to control all ledstrips is when every LEDstrip has its own data pin. This way it is possible for every LEDstrip to do something completely different, which would result in no boundaries for patterns. In the current project, we used an ESP32-C3-DevKitC-02.
        ...
        <li>
            <h3>Powersupply</h3>
                With 50 ledstrips, the power consumption will of course be a lot higher. This means that the power supply has to be more powerful. When we use 50 ledstrips, the max power consumption would be around 30 Watt. The project would have to be connected to a 5V 6A power supply like the LHA30F-5-SNY: <br></br>
                <img src="src/LHA30F-5-SNY.jpg" alt="LHA30F-5-SNY">
        </li>
        <li>
            <h3>PCB</h3>
        </li>
        To connect all LEDstrips, buttons and other components to the microcontroller, a PCB can be made. In our case, we placed a breadboard on top of the box that connects all components, which makes it easier to see what all our connections and components are when the project will be expanded.
        <li>
...
<h2>Buttons</h2>
The buttons were implemented fairly quickly, but we ran into some problems: they only worked with static functions, not with dynamic patterns and a delay occurred when pushed. We solved the delay problem by adding a debounce system that ensured that only one button press is registered. By working with threads we managed to cycle between dynamic functions.
<h1 id="image">Additional Images</h1>
<h2>Electric schematic</h2> 
First, we wanted to make a LEDcube using 2 data pins and 2 buttons. Because of the 2 data pins, we could send 2 different patterns to the LEDstrips which could make the illusion of falling rain or other dynamic patterns. The initial electric schematic looked like this:
